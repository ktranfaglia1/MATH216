...
```{r}
unifrom_sample<-rdunif(1000,1,30)
# basic plot
hist(unifrom_sample)
# correct the bining
hist(unifrom_sample, breaks=c(1,6,11,16,21,26,30),xlim=c(1,30))
...
```         
The histograms of the samples of size 1000 more stongly ressemble the the actual distribution of dU(1,30) relative to the sample of size 100. For the size 100 sample, there is more variance in the dirstibution from what should be an even dirstribution. The distribution is much closer to even for the sample of 1000 as more sample points reduce the impact of unlikely events, resulting in a much more similar distribution to what is expected.
```
...
```{r}
Binom_plotter(36,.35,17.4,36)
Binom_plotter(36,.35,17.6,36)
```

Is there any difference in the late 2 probabilities? why or why not?

```         
There is no difference between the last 2 probabilities. Since all the values in the distribution are discrete values, as in, non-continuous representations of data in an integer form, then altering the range for the probabiltiy by a decimal value which is not considered in the disrete data set will not impact the probability as if the value is greater than 17.4 or 17.6, this will only include the probbality of integer values greater than 18, the next discrete data point.
```
...
```{r}
n<-36
lambda<-seq(1,33,by=2)
x<-seq(0,36)
for (i in 1:length(p)){plot(x,dpois(x,lambda[i]),type="h",main=substitute(paste("n=36 lambda= ",a),list(a=lambda[i])))}

```

```         
The appears to be a right-skew for all values of λ 
```
...
Sample1 <- data$Sample_1
Sample2 <- data$Sample_2
Sample3 <- data$Sample_3

uniform_sample <- rdunif(100,0,20)
Poisson <- rpois(100,10)
Binomial <- rbinom(100, 50, 0.6)

hist(Sample1, xlim=c(min(Sample1) - 5, max(Sample1) + 5))
hist(Sample2, xlim=c(min(Sample2) - 5, max(Sample2) + 5))
hist(Sample3, xlim=c(min(Sample3) - 5, max(Sample3) + 5))

hist(uniform_sample, xlim=c(-5,25))
hist(Binomial, xlim=c(min(Binomial) - 5, max(Binomial) + 5))
hist(Poisson, xlim=c(min(Poisson) - 5, max(Poisson) + 5))

"Sample 1: Mean and Variance" 
mean(Sample1) 
var(Sample1)
"Sample 2: Mean and Variance" 
mean(Sample2) 
var(Sample2)
"Sample 3: Mean and Variance "
mean(Sample3) 
var(Sample3)
...
```         
By using the histograms and the descriptive statistics above we can come to the following conclusions:

Sample1 is the Binomial distribution (Bin(50,.6)) as it most closely resembles the histogram for a Bionamial distribution and each have a high mean of about 30 with a variance of 12.51677 for Sample1 and 10.27667 for the Binomial distribution.

Sample2 is the Poisson distribution (Pois(10)) as it most closley resembles the histogram for a Poisson distribution and each have a low mean of about 10 with a variance of 10.48485 for Sample2 and 7.121616 for the Poisson distribution.

Sample3 is the Uniform distribution (dU(0,20)) as it most closley resembles the histogram for a Uniform distribution and each have a low mean of about 10 with a very high variance of 10.48485 for Sample3 and 42.10697 for the Uniform distribution.
```
