...
2.  Obtain the cross-tabulation for Sex and Smokes.
    ...
    ```{r}
    CrossCount3 <- data2 %>% group_by(Sex, Smokes) %>%
      tally()

    CrossCount4 <- data2 %>% group_by(Sex, Smokes) %>%
      tally() %>%
      spread(Sex,n)
...
3.  Plot the bar chart of Smokes, group by Sex. (Recall that you do need to compute the CrossCount first.
    ...
    ```{r}
    ggplot(data=CrossCount3, aes(x=Smokes, y=n, fill= Sex))+
      geom_bar(stat="identity",position=position_dodge())+
      geom_text(aes(label=n), position = position_dodge(0.9), vjust=1.6)
    ```
...
The table suggests that there is a relationship between gender and smoking. According to the sample data, 8/35 = 0.228571 = 22.9% women smoke compared to 20/57 = 0.350877 = 35.1% men. Thus, the data suggests that men are more likely to have a smoking habit that women as men are 20/57 - 8/35 = 0.122306 = 12.2% more likely to smoke. More Concretely, There is a relationship between gender and smoking habits as they are not independent, they are dependent, such that P(S)!=P(S\|F), P(F)!=P(F\|S). It is important to note that the sample size is relatively small and that more men were sampled than women, still, the data presented reveals a relationship between gender and smoking habits.
